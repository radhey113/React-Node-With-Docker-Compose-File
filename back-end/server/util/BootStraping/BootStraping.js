...
exports.bootstrapAdmin = function (callback) {
    ...
    return Promise.all(defaultUser.map( USER => {
        ...
        CommonService.encryptPswrd(USER.password, (ERR, HASH)=> {
            if(ERR){
                throw ERR;
            }
            else {
        ...
        callback(CommonService.sendError(ERROR));
    });
};
...
exports.bootstrapAppVersion = function () {
    ...
    Promise.all(AppVersion.map(APP_V => {
        ...
        let OPTIONS = { upsert: true, new: true, setDefaultsOnInsert: true };
            ...
            return Models.AppVersion.findOneAndUpdate( CRITERIA, { $set: APP_V }, OPTIONS ).then(RESULT => {
